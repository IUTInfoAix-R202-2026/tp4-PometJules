...
public class PokemonController {
  ...
  private void initialize() {
    // 1. Dire à chaque colonne quelle donnée du Pokémon afficher.
    ...
    // 2. Abonner la TableView à la liste observable du ViewModel.
    table.setItems(viewModel.pokemonsProperty());

    // 3. Lier le label résumé au texte du ViewModel (sens unique).
    labelResume.textProperty().bind(viewModel.resumeProperty());

    // 4. Câbler le formulaire d'ajout.
